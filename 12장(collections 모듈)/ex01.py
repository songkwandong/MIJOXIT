...
print(deque_list.pop()) #오른쪽부터 
print(deque_list.popleft()) #왼쪽부터
print(deque_list)

print("=" * 40)
#appendleft() 메소드 사용. : 새로운 요소들을 왼쪽부터 입력되게 하여 
# 먼저 들어간 값부터 출력될 수 있게 한다, 즉, 양방향으로 데이터 입력이 가능.
deque_list.clear()
print(deque_list)
for i in range(5):
...
#연결 리스트는 데이터를 저장 할 때, 요소의 값을 한쪽으로 연결한 후
...
print("=" * 40)
#reversed() 메소드는 기존 요소를 반대로 저장할 수 있게 하는 것.
print(deque(reversed(deque_list)))

print("=" * 40)
#extend(), : 리스트를 우측에 통째로 붙힌다. 
# extendleft() : 리스트를 좌측에 통째로 붙힌다. - 방향은 반대로.
...
deque_list.clear()
basedata = ["a","b","c","d","e"]
# maxlen 매개변수는 deque의 사이즈를 고정시키는 인자값.
